...
public:
	...
	enum class ButtonsID
	{
		...
		R = 9,     //デジタルR
		//トリガー
		L2 = 10,	//Left Trigger -> Virtual Button
		R2 = 11,	//Right Trigger -> Virtual Button
		A = 12,     //デジタルA
		B = 13,     //デジタルB
		X = 14,     //デジタルX
		Y = 15,     //デジタルY
...
private:
	...
	{
	private:
		int keyCode_[(int)ButtonsID::max] = {
			VK_UP,
			VK_DOWN,
			VK_LEFT,
			VK_RIGHT,
		...
		//アナログパッド情報の取得
		Vec2 getThumb(const short xx, const short yy) const
		{
			if (!connected_) return Vec2(0, 0);
	...
	public:
		...
		void update()
		{
			...
			//Trigger as Button
			if (leftTrigger() > 0.6) now_ |= (1 << static_cast<int>(ButtonsID::L2));
			else now_ &= ~(1 << static_cast<int>(ButtonsID::L2));
			if (rightTrigger() > 0.6) now_ |= (1 << static_cast<int>(ButtonsID::R2));
			else now_ &= ~(1 << static_cast<int>(ButtonsID::R2));
		...
		void updateKeyboard()
		{
			...
			for (int i = 0; i < (int)ButtonsID::max; ++i) {
				ButtonsID buttonID = static_cast<ButtonsID>(i);
...
public:
	...
	//アナログスティック左の情報を取得する
	//1.0~-1.0で返ってくる
	Vec2 leftThumb(ID id = ID::controller_1P) const
	{
	...
	//アナログスティック右の情報を取得する
	//1.0~-1.0で返ってくる
	Vec2 rightThumb(ID id = ID::controller_1P) const
	{
	...
	//Debug用
	WORD getData(ID id = ID::controller_1P)
	{
		return controllers_[static_cast<int>(id)].getData();
	}
	...
	//アナログスティック左の情報を取得する
	//1.0~-1.0で返ってくる
	static Vec2 LeftThumb(ID id = ID::controller_1P)
	{
	...
	//アナログスティック右の情報を取得する
	//1.0~-1.0で返ってくる
	static Vec2 RightThumb(ID id = ID::controller_1P)
	{
	...
	//Debug用
	static WORD GetData(ID id = ID::controller_1P)
	{
		return GetInstance()->getData(id);
	}
};