...
//              2015.07.01  Ver1.02  植山沙欧  sleep()関連の関数を追加
//--------------------------------------------------------------------------------------------
#pragma once
...
#define DEBUG_OBJECT
#ifdef DEBUG_OBJECT
#  define DDOUT std::cout
#else
#  define DDOUT 0 && std::cout
#endif
...
// weak_ptr →  weak_ptr(継承クラスが対象)
template<typename T1, typename T2>
inline std::weak_ptr<T1> weak_cast(const std::weak_ptr<T2>& ptr)
{
	if (ptr.expired()) return std::weak_ptr<T1>();
	return std::static_pointer_cast<T1>(ptr.lock());
}
...
public:
    ...
    hierarchyLevel_(1)
  {
    initID();
    DDOUT.width(10);
    DDOUT << "Create: ID:" << ID() << " name:" << this->name() << std::endl;
  }
  ...
  {
    DDOUT.width(10);
    DDOUT << "Destroyed: ID:" << ID() << " name:" << this->name() << std::endl;
  }
...
//-----------------------------------------------------------------------
  template<typename Ptr>
  std::weak_ptr<Ptr> insertAsChild(Ptr* p)
  {
...
//-----------------------------------------------------------------------
  template<typename Ptr>
  std::weak_ptr<Object> insertToRoot(Ptr* o)
  {
	  auto root = getRootObject();
	  if (root.expired())
		  assert(!"ルートポインタがありませんよ");
	  root.lock()->insertAsChild(o);
	  return o->selfPtr();
  }
  //ポーズ状態でルートの子供に追加
  template<typename Ptr>
  std::weak_ptr<Object> insertToRootPause(Ptr* o)
  {
	  std::weak_ptr<Object> ret = insertToRoot(o);
	  o->pause();
	  return ret;
  }
  //スリープ状態でルートの子供に追加
  //寝かせるフレーム数を指定
  template<typename Ptr>
  std::weak_ptr<Object> insertToRootSleep(Ptr* o, int framecount)
  {
	  std::weak_ptr<Object> ret = insertToRoot(o);
	  o->sleep(framecount);
	  return ret;
...
//-----------------------------------------------------------------------
  ...
  //----- 追加 2016-06-13
  std::weak_ptr<Object> getObjectFromMyChildren(const std::string& objectName) const
  {
	  for (auto& child : children_)
	  {
...
//-----------------------------------------------------------------------
  ...
  //ルート以外で呼ばないでください
  virtual void renderLaterWithChildren()
  {
	  renderLaterSelect();
  }
  virtual void renderLater() {}; //render()系の処理後に処理する、通常描画の処理
  virtual void renderLaterPause() { renderLater(); }
  virtual void renderLaterSleep() { renderLater(); } //スリープ中の処理
  virtual void renderLaterDestroy() {} //削除予約中の処理
...
//-----------------------------------------------------------------------
  ...
  virtual void updateAfterChild() {} //子供の後の処理
...
//-----------------------------------------------------------------------
  ...
  //本当は描画の時にz軸の奥から手前まで順番にやらなきゃいけない。
  //以下のrenderLater系はツリー構造体描画のデバッグ
  void renderLaterWithChildrenNormal()
  {
	  renderLater();
	  renderLaterChildObjects();
  }
  void renderLaterPauseWithChildren()
  {
	  renderLaterPause();
	  renderLaterChildObjects();
  }
  void renderLaterSleepWithChildren()
  {
	  renderLaterSleep();
	  renderLaterChildObjects();
  }
  void renderLaterDestroyWithChildren()
  {
	  renderLaterDestroy();
	  renderLaterChildObjects();
  }
  void renderLaterSelect()
  {
	  switch (status())
	  {
	  case Status::run:
		  renderLaterWithChildrenNormal();
		  break;
	  case Status::pause:
		  renderLaterPauseWithChildren();
		  break;
	  case Status::sleep:
		  renderLaterSleepWithChildren();
		  break;
	  case Status::destroy:
		  renderLaterDestroyWithChildren();
		  break;
	  }
  }

  void renderLaterChildObjects()
  {
	  for (auto& child : children_)
	  {
		  child->renderLaterSelect();
	  }
  }
};
